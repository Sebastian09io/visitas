...
from django.utils.http import urlsafe_base64_encode  # Importa la función para codificar en base64 de manera segura para URLs
from django.utils.encoding import force_bytes  # Importa la función para forzar la codificación de una cadena
from django.contrib.auth.tokens import default_token_generator  # Importa el generador de tokens para la autenticación segura en el restablecimiento de contraseña
...
class PasswordResetTest(LiveServerTestCase):
    ...
    def test_password_reset_with_playwright(self):
        ...
        with sync_playwright() as p:
            ...
            # Genera el uidb64 y el token para el usuario
            uidb64 = urlsafe_base64_encode(force_bytes(self.user.pk))
            token = default_token_generator.make_token(self.user)

            # Simula el enlace de restablecimiento de contraseña
            reset_url = f'{self.live_server_url}/recuperar/{uidb64}/{token}/'
            page.goto(reset_url)

            # Llena los campos de la nueva contraseña y envía el formulario
            page.wait_for_selector('input[name="new_password1"]', timeout=10000)
            page.fill('input[name="new_password1"]', 'NuevaContraseñaSegura_123')
            page.fill('input[name="new_password2"]', 'NuevaContraseñaSegura_123')
            page.click('button[type="submit"]')
